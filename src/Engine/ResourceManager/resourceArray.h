...
public:
    void Insert(Resource resource, T&& data)
    {
        ...
        m_Resources[m_Count] = std::move(data);
    ...
    void Remove(Resource resource)
    {
        ...
        m_ResourceToIndex.erase(resource);

        if (removedIndex != m_Count - 1) {
            m_Resources[removedIndex] = std::move(m_Resources[m_Count - 1]);
    ...
    bool Contains(Resource resource) const
    {
        ASSERT(resource != INVALID_HANDLE);
