#include "defaultCameraControlSystem.h"
...
{
    ASSERT(ecs->HasComponent<Camera>(cameraEntity) && "Camera contol camer must have Camera component");

    m_Camera = cameraEntity;
    m_Sensitivity = sensitivity;
    m_Speed = speed;

    Camera& camera = ecs->GetComponent<Camera>(m_Camera);
    Transform& transform = ecs->GetComponent<Transform>(m_Camera);
...
void DefaultCameraControlSystem::Update(ECS *ecs, bool *keysDown, glm::vec2 mouseDelta, double dt)
{
