...
class RenderSystemV2 : public System {
public:
    ~RenderSystemV2();
    void Init(ECS *ecs, const RenderSystemInfo& info, const GraphicsFrontendInfo& frontendInfo, VulkanBackendV2 *backend);
    void Update(ECS *ecs, VulkanBackendV2 *backend);
    void RequestResize(VulkanBackendV2 *backend);
    void SetCamera(Entity camera);

    void UploadResources(std::vector<Resource> resources, const fs::path& sceneDir, ResourceManager *manager, VulkanBackendV2 *backend);
    ...
    Entity CreateSpotLight(ECS *ecs, const LightCreateInfo& info, VulkanBackendV2 *backend);
    Entity CreateDirectionalLight(ECS *ecs, const LightCreateInfo& info, VulkanBackendV2 *backend);
    Entity CreateText(ECS *ecs, const TextCreateInfo& info, VulkanBackendV2 *backend);

    Signature GetSignature(ECS *ecs) override { return ecs->GetBit<Transform>() | ecs->GetBit<Mesh>() | ecs->GetBit<Material>(); };

private:
    void AllocateShadowcaster(Light& light, Shadowcaster& shadowcaster, VulkanBackendV2 *backend);
    void UploadTexture(Resource resource, ResourceManager *manager, VulkanBackendV2 *backend);
    void UploadMaterial(Resource resource, ResourceManager *manager, VulkanBackendV2 *backend);
    void UploadModel(Resource resource, ResourceManager *manager, VulkanBackendV2 *backend);
    void UploadFont(Resource resource, ResourceManager *manager, VulkanBackendV2 *backend);
