...
VulkanBackendV2::~VulkanBackendV2()
{
...
void VulkanBackendV2::Init(struct GLFWwindow *window, Config &config)
{
...
void VulkanBackendV2::InitFrontend(GraphicsFrontend& frontend, const GraphicsFrontendInfo& info)
{
    // ================================================== Params ==================================================
...
void VulkanBackendV2::CleanupFrontend(GraphicsFrontend& frontend)
{
    frontend.camera = INVALID_HANDLE;

    for (TextureArrayV2& array : frontend.textureArrays) {
...
void VulkanBackendV2::Draw(ECS *ecs, GraphicsFrontend& frontend, const std::set<Entity>& entities)
{
    ...
    FrameDataV2 &frame = frames[frameIndex];
    ...
    SwapchainImageDataV2 &swapchainImage = m_Images[imageIndex];
...
AllocatedBuffer VulkanBackendV2::AllocateVertexBuffer(const Vertex *const vertices, uint32_t count)
{
...
AllocatedTexture VulkanBackendV2::AllocateTexture(ImageResource& image, GraphicsFrontend& frontend)
{
...
AllocatedTexture VulkanBackendV2::AllocateTexture(uint8_t *pixels, glm::ivec2 size, uint32_t flags, int32_t channels, GraphicsFrontend& frontend)
{
    ImageResource tmp{flags, size, pixels, channels};
    return AllocateTexture(tmp, frontend);
}

void VulkanBackendV2::Resize(ECS *ecs)
{
...
uint32_t VulkanBackendV2::AllocateShadowcaster(GraphicsFrontend& frontend)
{
    return frontend.shadowArray.Allocate();
}

void VulkanBackendV2::InitVulkan(struct GLFWwindow *window)
{
...
void VulkanBackendV2::InitDynamics()
{
    // ================================================== Swapchain ==================================================
    ...
    m_Images = std::vector<SwapchainImageDataV2>(swapchainImages.size());
    ...
    for (SwapchainImageDataV2& image : m_Images) {
...
void VulkanBackendV2::InitBuffers()
{
    // ================================================== Frame Buffers ==================================================
...
void VulkanBackendV2::InitSamplers()
{
...
void VulkanBackendV2::InitDescriptors()
{
...
void VulkanBackendV2::UpdateDescriptors()
{
    for (FrameDataV2& frame : frames) {
...
void VulkanBackendV2::InitPipelines()
{
    // ================================================== Push Constants ==================================================
    ...
    m_DepthPipeline = CreateGraphicsPipeline({
        .pipelineName = "Depth",
        .vertexShader = "depthV2.vert",
        .fragmentShader = "depthV2.frag",
        .viewport = &m_Viewport,
...
VkPipeline VulkanBackendV2::CreateGraphicsPipeline(const GraphicsPipelineCreateInfo&& info)
{
...
VkShaderModule VulkanBackendV2::LoadShaderModule(const std::filesystem::path& path)
{
...
void VulkanBackendV2::InitProfiling()
{
