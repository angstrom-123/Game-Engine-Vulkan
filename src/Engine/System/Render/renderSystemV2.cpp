...
void RenderSystemV2::Init(ECS *ecs, const RenderSystemInfo& info, const GraphicsFrontendInfo& frontendInfo, VulkanBackendV2 *backend)
{
...
void RenderSystemV2::Update(ECS *ecs, VulkanBackendV2 *backend)
{
...
void RenderSystemV2::RequestResize(VulkanBackendV2 *backend)
{
    backend->RequestResize(m_Frontend);
}

void RenderSystemV2::UploadResources(std::vector<Resource> resources, const fs::path& sceneDir, ResourceManager *manager, VulkanBackendV2 *backend)
{
    backend->WaitForIdle();
...
std::pair<Entity, std::vector<Entity>> RenderSystemV2::GetModel(ECS *ecs, const fs::path& modelRelativePath)
{
...
Entity RenderSystemV2::GetModelPart(ECS *ecs, const fs::path& modelRelativePath, const std::string& partName)
{
    ASSERT(!m_SceneDir.empty() && "Resources are not loaded");
...
Entity RenderSystemV2::CreatePointLight(ECS *ecs, const LightCreateInfo& info)
{
...
Entity RenderSystemV2::CreateSpotLight(ECS *ecs, const LightCreateInfo& info, VulkanBackendV2 *backend)
{
    Entity e = ecs->CreateEntity();
...
Entity RenderSystemV2::CreateDirectionalLight(ECS *ecs, const LightCreateInfo& info, VulkanBackendV2 *backend)
{
    Entity e = ecs->CreateEntity();
...
Entity RenderSystemV2::CreateText(ECS *ecs, const TextCreateInfo& info, VulkanBackendV2 *backend)
{
    Entity e = ecs->CreateEntity();
...
void RenderSystemV2::AllocateShadowcaster(Light& light, Shadowcaster& shadowcaster, VulkanBackendV2 *backend)
{
...
void RenderSystemV2::UploadTexture(Resource resource, ResourceManager *manager, VulkanBackendV2 *backend)
{
...
void RenderSystemV2::UploadMaterial(Resource resource, ResourceManager *manager, VulkanBackendV2 *backend)
{
    fs::path path = manager->GetPath(resource);
...
void RenderSystemV2::UploadModel(Resource resource, ResourceManager *manager, VulkanBackendV2 *backend)
{
    fs::path path = manager->GetPath(resource);
...
void RenderSystemV2::UploadFont(Resource resource, ResourceManager *manager, VulkanBackendV2 *backend)
{
    fs::path path = manager->GetPath(resource);
